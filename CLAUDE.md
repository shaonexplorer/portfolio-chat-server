...
- **Providers** – `src/app/provider/`
  ...
  - `neon-db.ts` creates a Neon PostgreSQL connection pool using the `pg` library with `DATABASE_URL`.
...
- **Resume ingestion utilities** – `src/app/utils/seed/`
  ...
  - `embedd.ts` stores each chunk as an embedding in Neon PostgreSQL via direct SQL queries. Automatically creates the table, functions, and detects embedding dimension at runtime.
...
- The `embedd.ts` automatically detects the embedding dimension and recreates the table with the correct schema.
